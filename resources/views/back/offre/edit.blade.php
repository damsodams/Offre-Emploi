...
<form style='margin-left:10px;' method="POST" action="{{route('offre.update', $offre->id)}}" enctype="multipart/form-data">
@csrf
@method('PUT')
  ...
  <div class="form-group">
     ...
     <select class="form-control" id="entreprise" name="entreprise">
       ...
       @endforeach
       <option value="{{$offre->entreprise->id}}" selected>{{ $offre->entreprise->nom }}</option>
     </select>
   </div>
